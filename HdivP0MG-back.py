...
# length(shape ratio) of the rectangular
L = 4
if dim == 2:
...
if dim == 2:
    W = HDiv(mesh, order=0, RT=True, dirichlet="wall|inlet")
elif dim == 3:
    W = HDiv(mesh, order=0, dirichlet="wall|inlet") # inconsistent option
M = TangentialFacetFESpace(mesh, order=0, dirichlet="wall|inlet")
fes = V * W * M 
